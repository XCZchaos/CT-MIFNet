...
from metric import GELU, Corr, LabelSmoothingLoss
from config_2b import config
...
from commom_spatial_pattern_2b import csp
...
class PatchEmbedding(nn.Module):
    def __init__(self, device, patch_sizeh: int = config.patchsize,
        ...
        self.projection = nn.Sequential(
            ...
            Conv2dWithConstraint(16, emb_size // 2, (8, 1), padding='same', max_norm=2.0),
...
class Add_Norm(nn.Module):
    def __init__(self, device, dropout_rate=0.3):
        ...
        self.dropout = nn.Dropout(dropout_rate).to(device)
        self.norm = nn.LayerNorm(80).to(device)
...
class Transformer(nn.Module):
    def __init__(self, device='cuda', depth=3, classes=4):
        ...
        self.fc = nn.Sequential(
            LinearWithConstraint(1280, 256, max_norm=2.0),
            nn.ELU(),
            nn.Dropout(0.5),
            nn.Linear(256, 32),
            nn.ELU(),
...
class Trans():
    def __init__(self, nsub):
        ...
        self.nSub = 9
        self.start_epoch = 0
        self.root = self.root = '/root/autodl-tmp/data/BCICIV_2b_gdf/'  # the path of data
    ...
    def get_source_data(self):
        ...
        # test data
        # to get the data of target subject
        self.test_tmp = scipy.io.loadmat(self.root + 'A0%dprocessing_E.mat' % self.nSub)
    ...
    def train(self):
        ...
        for e in range(self.n_epochs):
            ...
            for i, (img, label) in enumerate(self.dataloader):
                ...
                criterion = LabelSmoothingLoss(2, config.smooth)
            ...
            if (e + 1) % 1 == 0:
                self.model.eval()
                outputs_test, x_t2, x_f2 = self.model(test_data)
                criterion = LabelSmoothingLoss(2, config.smooth)
                loss_test = criterion(outputs_test, test_label)
        ...
        # torch.save(self.model.module.state_dict(), 'TFCformer_model_Subject_2b_%d.pth' % (self.nSub))
        averAcc = averAcc / num
